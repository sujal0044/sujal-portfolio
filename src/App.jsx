...
const NeuralNetwork = () => {
  ...
  useEffect(() => {
    ...
    // PERFORMANCE optimization: Lower particle count for raw speed
...
export default function App() {
  ...
  // DIRECT DOM REF FOR 144Hz (Bypassing React State completely)
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e) => {
      ...
      // Hardware accelerated translation running at max monitor refresh rate
      cursorRef.current.style.transform = `translate3d(${e.clientX}px, ${e.clientY}px, 0)`;

      // Detect hover over elements marked with .magnify and scale them while changing font weight
      const target = e.target;
      const magnifyTarget = target.closest('.magnify');
      const interactiveTarget = magnifyTarget || target.closest('a') || target.closest('button');
      
      if (interactiveTarget) {
        // Expands into a large, solid white circle
        cursorRef.current.classList.add('w-20', 'h-20', '-ml-10', '-mt-10', 'bg-white');
        cursorRef.current.classList.remove('w-4', 'h-4', '-ml-2', '-mt-2', 'mix-blend-difference', 'bg-white/90');
      } else {
        // Shrinks back to a sharp, mixed-blend white dot
        cursorRef.current.classList.add('w-4', 'h-4', '-ml-2', '-mt-2', 'mix-blend-difference', 'bg-white/90');
        cursorRef.current.classList.remove('w-20', 'h-20', '-ml-10', '-mt-10', 'bg-white');
      }
      
      // Apply the actual magnifying scale effect ONLY to what is inside the cursor
      if (magnifyTarget) {
        magnifyTarget.classList.add('font-bold', 'scale-[1.03]');
        magnifyTarget.classList.remove('font-light');
      } else {
        // Remove scale and bold from all .magnify elements when cursor leaves
        document.querySelectorAll('.magnify').forEach(el => {
          el.classList.remove('font-bold', 'scale-[1.03]');
  ...
  return (
    <div className={`bg-[#050505] text-white min-h-screen font-sans overflow-x-hidden selection:bg-[#ff6a00] selection:text-white relative ${isDesktop ? 'cursor-none' : ''}`}>
      ...
      {/* 144Hz ZERO-LAG TRUE LENS CURSOR */}
      {isDesktop && (
        <div 
          ref={cursorRef}
          className="fixed top-0 left-0 w-4 h-4 -ml-2 -mt-2 bg-white/90 rounded-full pointer-events-none z-[9999] mix-blend-difference transition-[width,height,margin,background] duration-200 ease-out will-change-transform"
        />
      )}
      ...
      {!loading && (
        <>
          {/* INTERACTIVE NEURAL NETWORK BACKGROUND (OPTION 1) */}
          <NeuralNetwork />

          {/* NAVIGATION */}
          <nav className="fixed top-0 left-0 w-full p-6 md:p-10 flex justify-between items-center z-50 pointer-events-none">
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="font-bold text-xl tracking-tighter pointer-events-auto magnify inline-block transition-all hover:scale-105 duration-300 hover:text-[#ff6a00] cursor-none">
              SUJAL<span className="text-[#ff6a00]">.</span>
            </motion.div>
            <motion.button initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} onClick={toggleMusic} className="pointer-events-auto cursor-none px-4 py-2 bg-[#111] border border-white/10 rounded-full hover:bg-white/10 transition-all duration-300 flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-[#ff6a00]">
          ...
          <section className="relative min-h-screen flex items-center justify-center pt-20 px-6 overflow-hidden">
            ...
            <div className="relative z-10 text-center max-w-5xl mt-10">
               <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="text-[#ff6a00] uppercase tracking-widest text-sm font-bold mb-8 magnify inline-block transition-all hover:scale-105 duration-300">
                 Creative Technologist • AI Enthusiast
               </motion.div>
               <br />
               <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="text-6xl md:text-8xl lg:text-9xl font-black tracking-tighter mb-8 leading-none magnify inline-block transition-all hover:scale-105 duration-300 hover:drop-shadow-[0_0_15px_rgba(255,106,0,0.3)]">
                 BUILDING <br /> THE <span className="text-[#ff6a00]">FUTURE.</span>
               </motion.h1>
               <br />
               <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7, duration: 1 }} className="text-gray-300 text-lg md:text-2xl max-w-3xl mx-auto font-light leading-relaxed magnify inline-block transition-all hover:scale-105 duration-300 hover:text-white">
          ...
          <div className="max-w-7xl mx-auto px-6 space-y-32 pb-40 relative z-10">
            ...
            <section className="pt-20 relative">
              ...
              />
              <TiltCard>
                <div className="bg-[#0a0a0a] border border-[#222] p-12 md:p-24 rounded-3xl relative overflow-hidden transition-colors hover:border-[#ff6a00]/30 shadow-2xl">
                  <div className="relative z-20">
                    <h2 className="text-4xl md:text-6xl font-black mb-10 tracking-tight magnify inline-block transition-all hover:scale-105 duration-300 hover:drop-shadow-[0_0_15px_rgba(255,106,0,0.3)]">The <span className="text-[#ff6a00]">Mindset.</span></h2>
                    <div className="grid md:grid-cols-2 gap-12 text-gray-300 text-lg leading-relaxed font-light">
                      <p className="magnify inline-block transition-all hover:scale-105 duration-300 hover:text-white">The architecture of code means nothing without the architecture of the mind. My journey from India to the heart of London's tech landscape wasn't just geographical; it was an evolution of extreme discipline. I operate at the intersection of rigorous logic and relentless creativity.</p>
            ...
            {/* TECHNICAL ARSENAL */}
            <section>
               <h2 className="text-3xl font-bold mb-12 flex items-center gap-4 magnify inline-flex transition-all hover:scale-105 duration-300 hover:text-white"><Code className="text-[#ff6a00]" /> Technical Arsenal</h2>
               <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                 {[
                   ...
                   <TiltCard key={i} className="h-full">
                     <div className="bg-[#0a0a0a] border border-[#222] p-10 rounded-2xl h-full hover:border-[#ff6a00]/50 transition-colors cursor-none magnify inline-block hover:scale-[1.03] duration-300 w-full hover:drop-shadow-lg">
            ...
            {/* SELECTED WORKS */}
            <section>
              <h2 className="text-3xl font-bold mb-12 flex items-center gap-4 magnify inline-flex transition-all hover:scale-105 duration-300 hover:text-white"><Layers className="text-[#ff6a00]" /> Selected Works</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {[
                  ...
                  <TiltCard key={i}>
                    <div className="bg-[#0a0a0a] border border-[#222] p-10 rounded-3xl h-full hover:border-[#ff6a00]/40 transition-all cursor-none group magnify inline-block hover:scale-[1.02] duration-300 w-full hover:drop-shadow-lg">
            ...
            {/* EXPERIENCE */}
            <section>
              <h2 className="text-3xl font-bold mb-12 flex items-center gap-4 magnify inline-flex transition-all hover:scale-105 duration-300 hover:text-white"><Terminal className="text-[#ff6a00]" /> Operational Experience</h2>
              <div className="space-y-6">
                {[
                  ...
                  { role: "Freelance Creative Developer", company: "Independent", year: "2023 - 2024", desc: "Architected modern web experiences, integrating headless CMS and secure Firebase authentication." }
                   ...
                   <div key={i} className="bg-[#0a0a0a] border border-[#222] p-8 rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center group cursor-none hover:border-[#ff6a00]/30 transition-all magnify hover:scale-[1.01] duration-300 hover:drop-shadow-md">
            ...
            <section className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
               {[
                 ...
                 <TiltCard key={i}>
                   <div className="bg-[#0a0a0a] border border-[#222] p-8 rounded-2xl cursor-none hover:bg-[#111] transition-colors magnify hover:scale-105 duration-300 hover:drop-shadow-lg">
            ...
            <section className="text-center pt-20 pb-20 relative z-10">
               <TiltCard>
                 <div className="bg-[#0a0a0a] border border-[#222] p-16 md:p-32 rounded-[4rem] relative overflow-hidden cursor-none hover:border-[#ff6a00]/40 transition-all duration-300 shadow-2xl">
                   ...
                   <h2 className="text-5xl md:text-8xl font-black tracking-tighter mb-6 relative z-10 text-white magnify inline-block transition-all hover:scale-105 duration-300 hover:drop-shadow-[0_0_15px_rgba(255,255,255,0.4)]">
